...
// Post Category
function real_estate_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'real_estate' ) . '</span></li>';
...
// Blog categories
function real_estate_featured_post_cat(){
...
// Post Tags
function real_estate_post_tags() {
...
// social media
if ( ! function_exists( 'real_estate_social' ) ) {
	function real_estate_social( $args = array() ) {
		...
		$url = real_estate_opt( 'real_estate_social_url' );
...
// header cart count
function real_estate_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( REAL_ESTATE_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'real_estate' ); ?>">
...
function real_estate_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'real_estate_contact7_form_content', 10, 2 );