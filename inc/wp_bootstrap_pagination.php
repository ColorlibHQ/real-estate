...
function real_estate_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'real_estate' ),
        'next_string'     => esc_html__( '&raquo;', 'real_estate' ),
        'before_output'   => '<div class="real_estate-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'real_estate_pagination_defaults', $defaults )
    );
